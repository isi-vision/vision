# Maintaining the vISIon website

Operational guide for the editorial board. The reader-facing site is at
https://isi-vision.github.io/vision/
...
```bash
git clone https://github.com/isi-vision/vision.git
cd vision
...
An article can be listed before its landing page exists. Give it a `"draft": true`
flag, with `title`, `subtitle`, `section`, `pages` and `start_page` filled in and
`authors`, `summary` and `points` left empty. It will appear on the articles index,
linking straight into the PDF at the right page, marked "Summary page coming soon".

To publish it properly: fill in `authors`, `summary` and `points`, delete the
`"draft": true` line, and re-run `python3 tools/build_articles.py`. The script
refuses to build a page whose summary, points or authors are empty, so you cannot
ship a half-finished one by accident.
